...
public abstract class AbTrie {
	...
	class TrieNode{
		...
		boolean remove(String s,int pos){
			...
			// 递归出口是已经到了字符串的最后一个字符，occurances=0，代表已经删除了
			// 否则继续递归到最后一个字符
			boolean ret;
			if(pos == s.length()-1){
		...
		TrieNode lookup(String s,int pos){
			...
			//如果找的次数已经超过了字符的长度，说明已经递归到超过字符串的长度，表明字符串不存在
			if(pos > s.length() || children ==null){
				return null;
				
			//如果刚好到了一个字符串的最后一个，则只需要返回最后一个字符对应的结点，若节点为空，则表明不存在该字符串
